function [filename]=Matfile_construct_WV_JEE(h5Path, OldWav, pl)
...
if classId ~= 0
    ...
    Spectroto= cell(nfiles*coeff,1);
    Spectrofo=cell(nfiles*coeff,1);
    histDur2=[];
    nsections = 0;
    Ag = zeros(1,10);
    Ag(1)=1;
       ...
       %Set up the counting matrices depending on the type of stim
        ...
        for i=1:length(on)
            ...
            elseif (laston~=0) && (on(i) == 0) || (laston~=0) && (i==length(on))
                ...
                if durInd>ceil(0.1*samprate)% make sure section is longer than 30ms of sound + 70ms silence
                    ...
                    if begInd==1 %the stim start by a vocalization
                        prezeros=zeros(ceil(0.01*samprate),1);%padd with zero before for 10ms
                        begInd_spike=-length(prezeros);
                    else
                        begInd_spike=begInd;
                    end
                    if endInd>length(sound_in) % the stim end by a vocalization ensure that we keep the tail of psth by extending the wavsection
                        postzeros=zeros(endInd-length(sound_in),1);
                        sec=[prezeros; sound_in(begInd:end);postzeros];
                        WavIndices{nsections}=[begInd length(sound_in)];
                    else
                        sec=[prezeros; sound_in(begInd:endInd);postzeros];
                    ...
                    Spectroto{nsections}= to;
                    Spectrofo{nsections}= fo;
                    Spectro{nsections}=VectorS;
                    
                    %% Isolate spikes that relate to the section and...
                    ...calculate average (psth) for this section. The length...
                        ... of the psth isolated depends on...
                        ...the length of the section
                    Win=ceil(SectionLength(nsections))/1000;%value Win given in sec
                    [Section_Trials,psth,stim_mean_rate, spike_std_rate, spike_zscore, pvalue, tvalue] = spikeTimes_psth_cal(begInd_spike, endInd, samprate,response,spike_rate_bg, Win);
    ...
    Res.Spectroto=Spectroto(1:nsections);
    Res.Spectrofo=Spectrofo(1:nsections);
